...
import {
  WorkerServiceEntity,
  ServiceType,
} from './modules/worker-service/entities/worker-service.entity';
...
async function runFakeData() {
  ...
  if (categories.length === 0) {
    console.log(
      '❌ Lỗi: Bạn cần chạy lệnh `npm run seed` 1 lần (dữ liệu mặc định) để có JobCategory trước khi tạo data ảo!',
    );
    process.exit(1);
  }
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const endTime = new Date(
      startTime.getTime() + randomInt(2, 8) * 3600 * 1000,
    );
  ...
  console.log(
    '✅ HOÀN TẤT! Đã thêm thành công: 5 Employers, 10 Workers, 15 Jobs, 15 WorkerServices.',
  );
  console.log(
    '🚀 Mẹo: Bây giờ bạn có thể gọi API GET /ai/dev-sync để đồng bộ toàn bộ đống dữ liệu này vào GraphRAG!',
  );
  process.exit(0);
}
